...
$result=mysql_query("SELECT * FROM product WHERE paspr=true ORDER BY date ASC LIMIT 5",$link);
if (mysql_num_rows($result)>1) {
	while($row=mysql_fetch_array($result))	
	{ 
...
<?php
$result=mysql_query("SELECT * FROM product WHERE paspr=true ORDER BY title ASC",$link);
